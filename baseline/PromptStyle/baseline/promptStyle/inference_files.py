...
def inference_one(stn_tst, style_embed):
    with torch.no_grad():
        x_tst = stn_tst.cuda().unsqueeze(0)
        x_tst_lengths = torch.LongTensor([stn_tst.size(0)]).cuda()
        sid = torch.LongTensor([7]).cuda()
        style_embed = style_embed.cuda()
...
if __name__ == "__main__":
    ...
    base_dir = f'outputs/promptstyle_vocoder7_spontaneous'
    os.makedirs(base_dir, exist_ok=True)
    
    # with open('/data/haoqiuyan/prompt_speech/code/PromptStyle/baseline/promptTTS/infer/spontaneous_verify_expresso.csv','r') as fp:
    with open('inputs/spontaneous.csv','r') as fp:
        ...
        for l in tqdm(lines):
            ...
            gen_wav = inference_one(stn_tst, style_embed)
            
            output_wav_file = f'{base_dir}/{item_name}_gen.wav'
            wavfile.write(output_wav_file, hps.data.sampling_rate, gen_wav)
